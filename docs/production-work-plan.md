...
- [ ] `prisma migrate` ناجح على MySQL
- [ ] `prisma generate` ناجح
- [ ] ملف `.env.example` بدون أسرار
...
**توقيع المبرمج:** ________  **التاريخ:** ________  
**إشعار واتساب `gate` أُرسل؟** نعم / لا  
**الانتقال للمرحلة 1 مسموح؟** نعم / لا
...
### الهدف
لوحة إدارة بشكل NexLink كاملة الهيكل الظاهر: RTL + Dark + شعار الراية + قائمة تنقل **كل روابطها تؤدي لصفحات موجودة** (حتى لو صفحة «قيد المرحلة» مؤقتة فقط إن كانت المرحلة الحالية لا تشملها — **الأفضل:** ابنِ الصفحات الهيكلية الفارغة برسالة واضحة «تُفعَّل في المرحلة X» بدل أزرار ميتة).
...
### مخرجات إلزامية
- [ ] `dir=rtl` + `lang=ar` + `data-theme=dark`
- [ ] Instrument Sans + primary `#5955D1`
- [ ] شعار استوديو الراية (ليس NexLink)
- [ ] `theme-btn` يعمل ويحفظ التفضيل
- [ ] لا طلبات إلى `nexlink.layoutdrop.com`
- [ ] السايدبار: عناصر المرحلة الحالية فقط ظاهرة وتعمل؛ الباقي مخفي حتى مرحلته

### اختبار يدوي — بوابة 1
| # | الاختبار | نتيجة |
|---|----------|--------|
| 1.1 | الصفحة RTL بالكامل | |
| 1.2 | Dark افتراضي بعد مسح التخزين | |
...
### مخرجات إلزامية
- [ ] مستخدم seed: بريد + كلمة مرور موثّقة في README داخلي للمبرمج فقط
- [ ] كلمة مرور خاطئة تُظهر خطأ واضح
- [ ] بعد الدخول يذهب لـ `/admin` أو التقويم حسب القرار
- [ ] زيارة `/admin/events` بدون جلسة تُرفض
...
### مخرجات إلزامية
- [ ] إنشاء / تعديل / عرض / حذف أو أرشفة خدمة
- [ ] إنشاء عرض بسعر `Decimal` صحيح
- [ ] تحقق حقول فارغة (لا حفظ ناقص)
...
| 9.5 | RTL + Dark + شعار على الشاشات الحرجة | |
