...
### قبل جلسة المهندسة (ممارسة معتمدة)
عند وجود تحديثات مهمة على `main`، ترسل الإدارة لنـهلة تنبيهاً ودّياً: **اسحبي الريبو قبل بداية الجلسة** (`git pull origin main`) ثم افتحي وكيل جديد.  
مرجع تجميع 2026-08-13: [pm-updates-2026-08-13.md](./pm-updates-2026-08-13.md)

### بداية / نهاية الجلسة
- **إدارة:** تقرير عمل (محور · منجز · بداية · نهاية · مدة)
- **نهلة:** رسالة ودّية قصيرة متغيّرة (أو `--text` من الوكيل)

```bash
node tools/pm-notify.mjs session-start --to both \
  --worked-on "المرحلة 4 — بوابة الزبائن والمناسبات" \
  --text "يلا نكمّل بوابة 4 بهدوء — اختبار يدوي أولاً."

node tools/pm-notify.mjs session-end --to both \
  --worked-on "زبائن + مناسبات" \
  --done "شاشات المرحلة 4 مبنية؛ بانتظار توقيع البوابة" \
  --text "شكراً على تركيزكِ اليوم — استريحي شوي."
```

الوكيل يفهم البداية/النهاية من:
1. **Hook:** فتح وكيل جديد / توقف الجلسة  
2. **كلمات المستخدم:** نبدأ، بداية جلسة… أو خلصنا، نهاية جلسة…  
التفاصيل: `.cursor/rules/session-lifecycle.mdc`  
منع التكرار: `SESSION_DEDUPE_MS` — أو `--force` عند الحاجة.
